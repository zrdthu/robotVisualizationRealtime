...
export class StickFigureComponent implements OnInit {
  ...
  updateVectorVisuals() {
    ...
    const orig_torsoVector = this.lineTorso.geometry.vertices[4].clone();
    const curTorsoVector = new THREE.Vector3(orig_torsoVector.x, orig_torsoVector.y, orig_torsoVector.z).applyQuaternion(this.lineTorso.quaternion);
    ...
    const upperRightArmVector = this.lineTorso.geometry.vertices[3].clone();
    const curRightUpperArmVector = new THREE.Vector3(upperRightArmVector.x, upperRightArmVector.y, upperRightArmVector.z).applyQuaternion(this.lineTorso.quaternion);
    ...
    const upperLeftArmVector = this.lineTorso.geometry.vertices[2].clone();
    const curLeftUpperArmVector = new THREE.Vector3(upperLeftArmVector.x, upperLeftArmVector.y, upperLeftArmVector.z).applyQuaternion(this.lineTorso.quaternion);
  ...
  initVector() {
    ...
    .then(geometry => {
      const material = new THREE.MeshPhongMaterial( { color: 0xFFD000 } );
      const mesh = new THREE.Mesh(geometry, material);
      this.scene.add(mesh);
      mesh.position.set(this.UPPER_ARM_LENGTH, this.SHOULDER_Y_POS + this.LOWER_ARM_Y_OFFSET, -this.UPPER_ARM_Z);
