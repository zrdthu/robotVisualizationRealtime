...
setInterval(() => {
  ...
  curSocket.msgStatus.numCheck += 1

  if (curSocket.msgStatus.numCheck < (config.POLL_INTERVAL / config.CHECK_INTERVAL)) {
    return
  }
  console.log(curSocket.msgStatus)
  if (curSocket.msgStatus.lastTimeStamp === 0) {
    ++curSockIndex
    ...
    curSocket.write(msg)
    curSocket.msgStatus.lastTimeStamp = timestamp
    curSocket.msgStatus.numSkip = 0
    curSocket.msgStatus.numCheck = 0
  } else {
    curSocket.msgStatus.numSkip += 1
  }
  // If current socket has not replied in 2 * interval, end it
  if (curSocket.msgStatus.numSkip > (config.POLL_INTERVAL / config.CHECK_INTERVAL)) {
    curSocket.destroy()
  ...
  //   var timestamp = microtime.now()
  //   msg.writeDoubleLE(timestamp, 0)
  //   console.log(`send message: ${msg.toString('hex')}`)
  //   curSocket.write(msg)
  // }
  // ++curSockIndex
  // if (curSockIndex >= clients.length) {
    ...
    if (lineEndIndex > -1) {
      ...
      for (let i = 0; i < clients.length; ++i) {
        ...
        if (getSocketId(client) === clientId) {
          ...
          client.msgStatus.numCheck = 0
          client.msgStatus.numSkip = 0
          break
        }
      }
      ...
      if (tinySync.keyPoints.length >= 6) {
        let keyPointIndex = {
